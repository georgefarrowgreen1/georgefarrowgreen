...
TINY = ("Qwen3-1.7B", "unsloth/Qwen3-1.7B-GGUF", "Qwen3-1.7B-Q4_K_M.gguf",
        "mlx-community/Qwen3-1.7B-4bit", "qwen3-1.7b")
MINI = ("Qwen3-4B", "unsloth/Qwen3-4B-GGUF", "Qwen3-4B-Q4_K_M.gguf",
...
SHAPES = [
    {"id": "tiny", "title": "Qwen3-1.7B",
     "sub": "One model, very small",
     "detail": "Runs on an 8GB Mac with room to spare. Below the size where "
               "tool calling is reliable, so expect drafts to need editing — "
               "but every mechanism is real and the queue behaves.",
     "gb": 1.1, "params": 1.7},
    {"id": "mini", "title": "Qwen3-4B",
     "sub": "One model, small",
     "detail": "The smallest worth pointing at real mail, and the sensible "
               "pick for 8-16GB Macs.",
     "gb": 2.5, "params": 4},
...
def build(shape: str, slots: int = 4, ctx_k: int = 32) -> list[dict]:
    ...
    spec = {"moe": LARGE, "dense": DENSE, "mini": MINI,
            "tiny": TINY}.get(shape, SMALL)
...
if __name__ == "__main__":               # still drives setup.sh
    ...
    shape = {"1": "small", "2": "moe", "3": "both",
              "4": "mini", "5": "tiny"}.get(choice, "small")
