...
| `python3 connect.py add messages local` | your own data, read-only |
| `python3 connect.py add ics_out local` | somewhere to put holds — the one that writes |
| `python3 connect.py add weather "Newcastle"` | a forecast. A lat and a lon leave; nothing else |
