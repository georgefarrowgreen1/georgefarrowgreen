...
const health=()=>{const open=store.approval.filter(a=>!a.decision).length;
  ...
  return {ok:true,at:`2026-08-22T0${HOUR}:02:00`,version:VERSION,
    ...
    spend:[{tokens:tok,max_tokens:4000000}]};};
const approvals=()=>store.approval.filter(a=>!a.decision).map(a=>{
  const t=policy.row(a.category);
  return {...a,stale:stale(a),pinned:!!(t&&t.pinned_manual),evidence:a.evidence||{}};});
const handled=()=>store.approval.filter(a=>a.decision).map(a=>({category:a.category,title:a.title,
  body:a.body,decision:a.decision}));
...
engine.start();
const ap=approvals();
chk(`approvals: ${ap.length} items, each has body/title/evidence/stale/pinned`,
    ap.length>0 && ap.every(a=>a.body&&a.title&&a.evidence&&'stale'in a&&'pinned'in a));
chk('one is pinned (renders amber with Later/Open thread)', ap.some(a=>a.pinned));
const rn=runs();
chk(`runs: ${rn.length}, each has workflow/status/steps/writes and result parses`,
    rn.every(r=>r.workflow&&r.status&&'steps'in r&&'writes'in r&&JSON.parse(r.result)));
...
policy.record('availability_reply','approve');
const [ok,why]=policy.mayAct('availability_reply');
