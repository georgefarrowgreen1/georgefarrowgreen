...
function seed(trust){
  const s=new Store(); s.workspace=Object.keys(WN).map(id=>({id,name:WN[id]}));
...
let [s,p,e]=seed(); s.workspace.forEach(w=>e.start(w.id));
chk('4 workspaces swept, '+s.approval.filter(a=>!a.decision).length+' queued for you',
    s.approval.filter(a=>!a.decision).length===4);
chk('injected email quarantined and given no draft',
    s.run.find(r=>r.workspace_id==='cottages').result.flagged===1);
p.record('cottages','availability_reply','approve');
chk('availability_reply -> '+p.mayAct('cottages','availability_reply')[1],
    p.mayAct('cottages','availability_reply')[0]);
const kept=s.trust;
[s,p,e]=seed(kept); e.start('cottages');
...
[s,p,e]=seed(); e.crashAt='draft'; e.start('cottages'); e.crashAt=null;
