...
MUTATIONS = [
    ...
    dict(probe="A115", why="the allowed shim answers for the blocked python",
         file="core/doctor.py",
         find='    if "block" in verdicts:\n        return "block"',
         replace='    if verdicts:\n        return verdicts[0]'),
    ...
    dict(probe="A131", why="the sources sheet loses its door to the pane",
         ...
         find='id="fda-open"',
         replace='id="fda-shut"'),
    dict(probe="A132", why="a message body can end its own literal",
         file="core/connectors/messages_out.py",
         find="    return '\"' + text.replace(\"\\\\\", \"\\\\\\\\\")"
              ".replace('\"', '\\\\\"') + '\"'",
         replace="    return '\"' + text + '\"'"),
    dict(probe="A132", why="a recipient list is one recipient again",
         file="core/connectors/messages_out.py",
         find='    if "," in v or ";" in v:',
         replace='    if False and ("," in v or ";" in v):'),
    dict(probe="A132", why="the draft's recipient stops being fixed",
         file="core/connectors/messages_out.py",
         find='    if expected and handle(expected, "expected recipient") '
              '!= to:',
         replace='    if False and expected:'),
    dict(probe="A132", why="the send gate asks the wrong app's door",
         file="core/actions.py",
         find='permission.require(store, permission.APP, "Messages",',
         replace='permission.require(store, permission.APP, "Calendar",'),
    dict(probe="A132", why="every recipient is mailed, phone numbers "
                           "included",
         file="core/actions.py",
